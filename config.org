...
#+BEGIN_SRC emacs-lisp
  ...
  (defun y/project-set-default()
    ...
    (let ((de '())
      ...
      ;; append dl to include path if not absolute path
      (dolist (d y/project-include-path)
        (if (stringp d)
            (progn
              (or (string-prefix-p "/" d)
                  (setq d (concat dl d)))
              (add-to-list 'de (file-truename d)))
          (warn "y/projects: not a string include path:" d)))
      ;; Alwasys add root directory
      (and dl (add-to-list 'de (file-truename dl)))

      (setq semantic-c-dependency-system-include-path de)
      (setq semanticdb-project-roots de)
      ...
      (when de
        (if (string= major-mode "c-mode")
            (setq de (append de y/semantic-c-system-include-path))
          (setq de (append de y/semantic-c++-system-include-path))))
      (setq flycheck-clang-include-path de
            flycheck-clang-includes de
            flycheck-gcc-include-path de
            company-c-headers-path-system de)))
  ...
  (defun y/project-set-linux-kernel()
    ...
    (interactive)
    (message "Set to linux %s" (buffer-file-name))
    (setq buffer-read-only t)
    (y/whitespace-linux-kernel-style)
    (setq y/project-include-path
          (append y/project-include-path
