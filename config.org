...
#+BEGIN_SRC emacs-lisp
  (dolist (x '("Source Code Variable" "Source Code Pro"))
          (when (find-font (font-spec :name x :size 15))
...
#+BEGIN_SRC emacs-lisp
  (use-package expand-region
    ...
    ("C-c e s"   . er/mark-symbol)
    ("C-c e f"   . er/mark-method-call)
    ("C-c e i q" . er/mark-inside-quotes)
    ("C-c e o q" . er/mark-outside-quotes)
    ("C-c e i p" . er/mark-inside-pairs)
    ("C-c e o p" . er/mark-outside-pairs)
    ("C-c e i t" . er/mark-inner-tag)
    ("C-c e p t" . er/mark-outer-tag))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; search <header.h> from system include path, and search
  ;; "header.h" from include path.
  (defvar-local y/project-include-path nil
    "Project additional include path.")
  (defvar-local y/project-system-include-path nil
    "Project additional system include path.")
  (defvar-local y/project-tab-visible t
    "Project source code show visible TAB or not.")

  (defvar y/project--buffer-changed nil
    "Project internal used to check buffer switched or not.")
  ...
  (defun y/semantic-set-default-include-path()
    ...
    (setq-mode-local c++-mode semantic-dependency-system-include-path
                     y/semantic-c++-system-include-path)
    (setq-mode-local c-mode semantic-dependency-include-path nil)
    (setq-mode-local c++-mode semantic-dependency-include-path nil))
  ...
  (defun y/project-set-config(&optional system-include-path-clear)
    "Set project include path.

  SYSTEM-INCLUDE-PATH nil do nothing, t clear system include path."
    ...
    (let ((de '()) ;; directory external include path
      ...
      (unless system-include-path-clear
  ...
  (defun y/project-set-linux-kernel()
    ...
    (interactive)
    (and (string= major-mode "c++-mode")
         (c-mode)) ;; force to c-mode for linux
    (setq buffer-read-only t
          y/project-tab-visible nil)
    (setq y/project-system-include-path
          '("include" "include/uapi" "arch/x86/include"))
    (y/project-set-config t))
  ...
  ;; Follow hooks not work properly, use post-command-hook replaced.
